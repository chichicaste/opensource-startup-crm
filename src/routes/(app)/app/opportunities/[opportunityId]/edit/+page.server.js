...
export const actions = {
	...
	default: async ({ request, params, locals }) => {
		...
		const expectedRevenue = form.get('expectedRevenue')
			? parseFloat(form.get('expectedRevenue')?.toString() || '')
			: null;
		const stage = form.get('stage')?.toString();
		const probability = form.get('probability')
			? parseFloat(form.get('probability')?.toString() || '')
			: null;
