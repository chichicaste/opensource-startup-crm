...
import { industries, accountTypes, accountOwnership, ratings, countries } from '$lib/data/index.js';
	...
	default: async ({ request, locals }) => {
		...
		const accountData = {
			...
			numberOfEmployees: formData.get('numberOfEmployees')
				? parseInt(formData.get('numberOfEmployees')?.toString() || '0')
				: null,
			annualRevenue: formData.get('annualRevenue')
				? parseFloat(formData.get('annualRevenue')?.toString() || '0')
				: null,
