				...
				<div class="mb-6">
						...
						use:enhance={({ submitter }) => {
							...
							return async ({ result, update }) => {
								if (result.type === 'success') {
									message =
										/** @type {string} */ (result.data?.message) || 'Successfully unsubscribed!';
									showMessage = true;
									isSuccess = true;
					...
					>
						...
						<div class="space-y-4">
							<button
								type="submit"
								class="flex w-full justify-center rounded-md border border-transparent bg-red-600 px-4 py-3 text-sm font-medium text-white shadow-sm transition-colors duration-200 hover:bg-red-700 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
							>
								Confirm Unsubscribe
							</button>
